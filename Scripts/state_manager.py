...
def save_checkpoint(checkpoint_dict: dict):
    """Upsert checkpoint into Snowflake. Silently skips if table doesn't exist."""
    try:
        conn   = get_conn()
        cursor = conn.cursor()
        now    = datetime.utcnow()
        ...
        print(f"⚠️  save_checkpoint failed: {e}")
        print("    State saved to local file only.")
...
def load_checkpoint() -> dict:
    ...
    conn.close()
    if not row:
        raise FileNotFoundError("No checkpoint in Snowflake. Run full load first.")
    return json.loads(row[0])


def save_open_invoices(invoices: list):
    """Replace open invoices in Snowflake. Silently skips if table doesn't exist."""
    try:
        conn   = get_conn()
        cursor = conn.cursor()
        now    = datetime.utcnow()
        ...
        print(f"⚠️  save_open_invoices failed: {e}")
        print("    State saved to local file only.")
