"""
================================================================================
Financial Data Warehouse - INCREMENTAL DAILY GENERATOR v2.1
Compatible with full_Load.py v6.0
Now generates: FACT_INVOICE_HEADER, FACT_INVOICE_LINE_ITEM, 
...
class Checkpoint:
    ...
    def load(cls) -> 'Checkpoint':
        ...
        if not os.path.exists(CHECKPOINT_FILE):
            raise FileNotFoundError(f"No checkpoint found. Run full_Load.py first.")
            ...
            if 6 <= days_since_due <= 15:
                if random.random() < 0.10:
                    if random.random() < 0.85:
                        ...
                        return round(remaining * random.uniform(0.5, 0.8), 2)
            ...
            if 16 <= days_since_due <= 30:
                if random.random() < 0.067:
                    if random.random() < 0.70:
                        ...
                        return round(remaining * random.uniform(0.3, 0.6), 2)
...
class ReturnSimulator:
    ...
    def should_return(invoice_amount: float, customer_type: str, days_since_invoice: int) -> bool:
        """Determine if a return should happen on this invoice"""
        # Base return probability
        if customer_type == 'B2C':
            base_prob = 0.03  # 3% of invoices have returns
        elif customer_type == 'B2D':
            base_prob = 0.02  # 2% for B2D
        else:  # B2B
            base_prob = 0.01  # 1% for B2B
        
        # Higher probability for larger invoices
        if invoice_amount > 100000:
            base_prob *= 1.5
        elif invoice_amount > 50000:
            base_prob *= 1.2
        
        # Returns more likely within first 7 days
    ...
    def calculate_return_amount(invoice_amount: float, paid_amount: float) -> float:
        """Calculate return amount - typically full or partial"""
        if paid_amount <= 0:
            return 0.0
        
        # 70% full return, 30% partial return
        if random.random() < 0.7:
            return min(invoice_amount, paid_amount)
        else:
            # Partial return: 20-80% of paid amount
            return round(paid_amount * random.uniform(0.2, 0.8), 2)


# ============================================================================
# FULL LOAD RUNNER (references your full_Load.py)
# ============================================================================

class FullLoadRunner:
    def __init__(self, generator_class):
        self.GeneratorClass = generator_class

    def run(self):
        logger.info("=" * 60)
        logger.info("FULL LOAD - generating historical data")
        logger.info("=" * 60)
...
class DailyRunner:
    def __init__(
        ...
        employees: List[Dict],
        locations: List[Dict],
        credit_policies: List[Dict],
        credit_mappings: List[Dict],
        ...
        for c in self.customers:
            ...
            if ctype in ('B2B', 'B2D'):
                pool = [s for s in self.stores
                        if self.loc_state.get(self.store_loc.get(int(s['store_key']))) == cstate]
                if pool:
                    rng = random.Random(ckey)
                    self.cust_primary_store[ckey] = int(rng.choice(pool)['store_key'])
                else:
                    rng = random.Random(ckey)
                    self.cust_primary_store[ckey] = int(rng.choice(self.stores)['store_key'])
            else:
    ...
    def run(self):
        ...
        # Step 1: Settle old invoices and generate returns
        logger.info("Step 1: Settling old open invoices & processing returns...")
        settled_keys = []
        ...
        for inv in self.store.all():
            ...
            if pay_amount is not None and pay_amount > 0:
                ...
                pay_rec = {
                    ...
                    'remarks': f"Payment for old invoice {inv['invoice_id']}",
                }
            ...
            # Process returns for this invoice (if any)
            ...
            if ReturnSimulator.should_return(inv['original_amount'], inv['customer_type'], days_since_invoice):
                ...
                if return_amount > 0:
                    ...
                    return_rec = {
                        ...
                        'remarks': f"Return processed for invoice {inv['invoice_id']}",
                    }
                    returns.append(return_rec)
                    
                    # Update customer outstanding if return is on B2B/B2D invoice
                    if inv['customer_type'] in ('B2B', 'B2D'):
                        self.cp.customer_outstanding[int(inv['customer_key'])] = max(0.0, 
                            self.cp.customer_outstanding[int(inv['customer_key'])] - return_amount)
                    
                    # Update invoice remaining balance if return is processed
                    if return_amount <= inv['remaining_balance']:
                        self.store.update(inv['invoice_key'], -return_amount)  # Negative to reduce balance
                        inv['remaining_balance'] += return_amount  # Actually increases remaining balance
                    
                    logger.debug(f"  Return generated: {return_id} for invoice {inv['invoice_id']} - amount: {return_amount}")
        ...
        # Step 2: Generate new invoices with line items
        logger.info("Step 2: Generating today's new invoices and line items...")
    ...
    def _gen_todays_invoices(self, today: date):
        ...
        for ctype, count_range in INVOICES_DAILY.items():
            ...
            for _ in range(count):
                ...
                if policy and ctype in ('B2B', 'B2D'):
                    block = str(policy.get('block_on_exceed', 'True')).lower() in ('true', '1')
                    if block:
                        limit = float(policy.get('credit_limit_amount', 0))
                        outstanding = self.cp.customer_outstanding.get(ckey, 0.0)
                ...
                if pstore_key and random.random() < 0.8:
                    ...
                    store = random.choice(self.store_list)
                ...
                for prod in sel_prods:
                    ...
                    line_items_for_invoice.append(line_item)
                ...
                if invoice['total_invoice_amount_incl_gst'] < min_val:
                    ...
                    for li in line_items_for_invoice:
                        for field in ['gross_amount', 'discount_amount', 'taxable_amount', 
                            ...
                            li[field] = round(li[field] * scale_factor, 2)

                inv_total = invoice['total_invoice_amount_incl_gst']

                # Add line items to collection
        ...
        # Update line_item sequence in checkpoint
...
class BackfillRunner:
    ...
    def run(self):
        ...
        while current <= today:
            logger.info(f"\n{'─'*60}")
            logger.info(f"Backfill: {current}")
...
def load_dimensions(full_load_dir: str = FULL_LOAD_DIR) -> Dict[str, List[Dict]]:
    tables = [
        'DIM_CUSTOMERS', 'DIM_PRODUCTS', 'DIM_STORE',
        'DIM_USER', 'DIM_LOCATION',
...
def main():
    parser = argparse.ArgumentParser(description='Incremental Generator v2.1')
    ...
    if args.mode == 'full':
        try:
            ...
            return
        FullLoadRunner(FinancialDataGenerator).run()
    else:
        ...
        if args.mode == 'daily':
            if run_date:
                runner_kwargs['run_date'] = run_date
            DailyRunner(**runner_kwargs).run()
        elif args.mode == 'backfill':
            BackfillRunner(**runner_kwargs).run()
