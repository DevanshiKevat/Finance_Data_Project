...
def load_dimensions_from_snowflake() -> Dict:
    """
    Fallback: read dimension tables directly from Snowflake RAW schema.
    Called automatically when local CSVs are not found (e.g. ephemeral CI runners).

    Note: DIM_USER in Snowflake maps to DIM_USER key used by DailyRunner
    (full_Load.py generates DIM_USER.csv; upload_to_snowflake.py loads it
    into the DIM_EMPLOYEE table — so we read DIM_EMPLOYEE here and return
    it under the 'DIM_USER' key so the rest of the code is unaffected).
    """
    import snowflake.connector

    # Maps the key DailyRunner expects → actual Snowflake table name in RAW
    TABLE_MAP = {
        'DIM_CUSTOMERS':           'DIM_CUSTOMERS',
        'DIM_PRODUCTS':            'DIM_PRODUCTS',
        'DIM_STORE':               'DIM_STORE',
        'DIM_USER':                'DIM_EMPLOYEE',   # CSV name differs from table name
        'DIM_LOCATION':            'DIM_LOCATION',
        'CREDIT_POLICY':           'CREDIT_POLICY',
        'CUSTOMER_CREDIT_MAPPING': 'CUSTOMER_CREDIT_MAPPING',
    }
    ...
    for key, table in TABLE_MAP.items():
        try:
            ...
            result[key] = rows
            logger.info(f"  Loaded {key} from Snowflake ({table}): {len(rows):,} rows")
        except Exception as e:
            logger.warning(f"  Could not load {key} from Snowflake: {e}")
            result[key] = []
    cursor.close()
    conn.close()
    return result
...
def load_dimensions(full_load_dir: str = FULL_LOAD_DIR) -> Dict:
    ...
    for t in tables:
        ...
        if os.path.exists(path):
            ...
            logger.warning(f"  {t}.csv not found locally")
            result[t] = []
            any_missing = True

    if any_missing:
        logger.info("  CSV files missing — falling back to Snowflake for all dimensions...")
        try:
            result = load_dimensions_from_snowflake()
        except Exception as e:
            logger.error(f"  Snowflake dimension fallback also failed: {e}")
            logger.error("  Cannot run without dimension data. Aborting.")
            raise RuntimeError("No dimension data available from CSV or Snowflake.") from e
